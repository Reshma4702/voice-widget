...
(function () {
  ...
  function createWidget(root) {
    ...
    var welcomeEffect = createEffect(function () {
      /* Greeting (both the two seeded messages AND the spoken audio) fires
         exactly once per browser tab. sessionStorage survives page reloads
         within the same tab but clears when the tab is closed, which matches
         the intended UX: no re-greet on minimize/reopen, fresh greet in a
         new tab. `greeted` is the in-page guard so a single load never
         re-seeds after the first open. */
