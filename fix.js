...
(function () {
  ...
  /* Speech-only URL scrubbing and display-side link normalization. */
  var URL_RE_SPEECH = /(?:https?:\/\/|www\.)[^\s<]+[^\s<.,:;"')\]]/gi;
  function stripForSpeech(text) {
    if (!text) return "";
    return String(text)
      .replace(URL_RE_SPEECH, " You can check it here ")
      .replace(/\s{2,}/g, " ")
      .trim();
  }
  ...
  var URL_RE_DISPLAY = /(?:https?:\/\/|www\.)[^\s<)]+[^\s<.,:;"')\]]/gi;
  function linkifyForDisplay(text) {
    if (!text) return "";
    var src = String(text);
    // Skip if already contains markdown/html links to avoid double wrapping.
    if (/\]\(https?:|<a\s/i.test(src)) return src;
    return src.replace(
      /(check(?:\s+it)?\s+here[\s,:.-]*)?((?:https?:\/\/|www\.)[^\s<)]+[^\s<.,:;"')\]])/gi,
      function (_m, phrase, url) {
        var href = normalizeHref(url);
        var label = phrase ? phrase.replace(/[\s,:.\-]+$/, "") : "check here";
        return "[" + label + "](" + href + ")";
      }
    );
  }
