...
(function () {
  ...
  function createWidget(root) {
    ...
    var welcomeEffect = createEffect(function () {
      /* Greeting runs once per page load.
           - closing and reopening the widget does NOT replay it, because
             `greeted` is never reset on close;
           - a refresh, or closing the tab and opening it again, DOES replay it,
             because `greeted` lives only in memory and dies with the page.
         Deliberately not sessionStorage: that survives a refresh, which is the
