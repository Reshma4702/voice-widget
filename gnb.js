...
(function () {
  ...
  var GREETING_SESSION_KEY = "agilisium_greeted";
  var CHAT_SESSION_KEY = "agilisium_chat_session";
  var MAX_PERSISTED_MESSAGES = 60;
  var MOBILE_BREAKPOINT = 768;

  /* Painted inline only if the stylesheet fails to give the pill a colour. */
  var LAUNCHER_FALLBACK_BG = "#000000";

  /* Session-pinned TTS voice — one voice for the whole session, no fallback. */
  var TTS_VOICE_ID = "nova";
  var TTS_SESSION_KEY = "agilisium_tts_session";
  function getVoiceSession() {
    try {
      var raw = sessionStorage.getItem(TTS_SESSION_KEY);
      if (raw) {
        var parsed = JSON.parse(raw);
        if (parsed && parsed.voiceId === TTS_VOICE_ID && typeof parsed.seed === "number") {
          if (!Array.isArray(parsed.previousRequestIds)) parsed.previousRequestIds = [];
          return parsed;
        }
      }
    } catch (e) {}
    var fresh = {
      ...
      seed: Math.floor(100000000 + Math.random() * 900000000),
      previousRequestIds: []
    };
    try { sessionStorage.setItem(TTS_SESSION_KEY, JSON.stringify(fresh)); } catch (e) {}
    return fresh;
  }
  function pushVoiceRequestId(id) {
    if (!id) return;
    var sess = getVoiceSession();
    sess.previousRequestIds.push(id);
    if (sess.previousRequestIds.length > 3) {
      sess.previousRequestIds = sess.previousRequestIds.slice(-3);
    }
    try { sessionStorage.setItem(TTS_SESSION_KEY, JSON.stringify(sess)); } catch (e) {}
  }

  /* ----------------------------------------------------------------- links
     Two separate jobs. In SPEECH a URL must never be read out loud, so it is
     swapped for a short phrase. On SCREEN the same URL becomes a real
     clickable link with friendly label text. Change the two constants below
     to reword either side. */
  var LINK_SPEECH_PHRASE = "the link on screen";
  var LINK_DISPLAY_LABEL = "Take a look";

  /* Matches a URL in either of the two forms the model actually emits:
     a full one with a scheme or www prefix, OR a bare domain such as
     "agilisium.com/privacy". The bare form matters because the sentence
     chunker splits on periods: a URL cut after "www." leaves a scheme-less
     tail in the next chunk, and without this branch that tail was spoken
     aloud one word at a time. Requires a real TLD so ordinary prose like
     "U.S." or "etc." is never mistaken for an address. */
  var URL_TOKEN =
    "(?:https?:\\/\\/|www\\.)[^\\s<]+" +
    "|(?:[a-z0-9][a-z0-9-]*(?:\\.[a-z0-9-]+)*" +
    "\\.(?:com|net|org|io|ai|co|in|edu|gov|dev|app|us|uk|eu)" +
    "(?:\\/[^\\s<]*)?)";
  function urlRe() {
    return new RegExp(URL_TOKEN, "gi");
  }

  /* Trailing sentence punctuation isn't part of the address — keep it so the
     spoken sentence still ends properly. */
  function splitTrailingPunctuation(match) {
    var m = String(match).match(/[.,:;!?"')\]]+$/);
    if (!m) return { url: String(match), trail: "" };
    return { url: String(match).slice(0, -m[0].length), trail: m[0] };
  }

  /* Keeps a chunk boundary from landing inside a URL. The sentence splitter
     treats every "." as a possible sentence end, which cuts an address in
     half and strands its tail in the next chunk. Given a candidate cut index
     this returns a URL-safe one: just past the address if it has already
     finished, or at its start if it may still be streaming in. */
  function snapCutOutOfUrl(str, cut) {
    var re = urlRe();
    var m;
    while ((m = re.exec(str)) !== null) {
      var start = m.index;
      var end = start + m[0].length;
      if (cut > start && cut < end) return end < str.length ? end : start;
      if (start >= cut) break;
    }
    return cut;
  }
  ...
  function stripForSpeech(text) {
    ...
    return String(text)
      .replace(urlRe(), function (m) {
        var parts = splitTrailingPunctuation(m);
        if (!parts.url) return m;
        return " " + LINK_SPEECH_PHRASE + parts.trail + " ";
      })
      .replace(/\s{2,}/g, " ")
      .trim();
  }
  ...
  /* Call-to-action wording the model tends to put in front of a URL. When
     present it is absorbed into the link text instead of being left stranded
     next to it. */
  var LINK_CTA =
    "(?:you\\s+can\\s+)?(?:check(?:\\s+it)?(?:\\s+out)?|take\\s+a\\s+look|have\\s+a\\s+look|see|read\\s+more|learn\\s+more|find\\s+(?:it|out)\\s+more?)\\s*(?:here|at)?[\\s,:.-]*";

  function linkifyForDisplay(text) {
    if (!text) return "";
    var src = String(text);
    /* Skip if it already contains markdown/HTML links, to avoid double wrapping. */
    if (/\]\(https?:|<a\s/i.test(src)) return src;
    var re = new RegExp("(" + LINK_CTA + ")?(" + URL_TOKEN + ")", "gi");
    return src.replace(re, function (_m, phrase, url) {
      var parts = splitTrailingPunctuation(url);
      if (!parts.url) return _m;
      var label = phrase ? phrase.replace(/[\s,:.\-]+$/, "") : LINK_DISPLAY_LABEL;
      if (!label) label = LINK_DISPLAY_LABEL;
      return "[" + label + "](" + normalizeHref(parts.url) + ")" + parts.trail;
    });
  }


  /* Gap left between the host site's sticky header and the panel. */
  /* When true the host navbar is left un-dimmed and clickable while the chat
     is open, on every page. Set false to blur the whole viewport instead. */
  var KEEP_NAVBAR_CLEAR = true;
  ...
  var MIN_W = 300, MIN_H = 420, MAX_W = 640, MAX_H = 760;
  ...
  /* A refresh and a link click to another page are both just "a new document"
     unless we ask the browser which it was. Navigation Timing tells them
     apart, which is what lets the greeting replay on reload but stay quiet
     while someone browses from page to page in the same tab. */
  ...
  /** Restores the transcript so a conversation survives page navigation. */
  ...
  function saveSessionMessages(msgs) {
    try {
      var trimmed = msgs.slice(-MAX_PERSISTED_MESSAGES).map(function (m) {
        return { role: m.role, content: m.content, ts: m.ts, voice: !!m.voice };
      });
      sessionStorage.setItem(CHAT_SESSION_KEY, JSON.stringify(trimmed));
    } catch (e) {}
  }

  /* On an explicit refresh the visitor is asking to start over, so both the
     greeting flag and the stored transcript go. Plain navigation keeps them. */
  function resetSessionOnReload() {
  ...
  async function transcribe(blob) {
    ...
    form.append("file", blob, "input." + extensionForMime(blob && blob.type));
  ...
  /* MediaSource lets playback start on the first bytes rather than waiting for
     the whole MP3 to download. Older iOS Safari has no audio MSE, hence the
     probe and the buffered fallback inside synthesize. */
  var MSE_SUPPORTED =
  ...
  async function synthesize(text, signal) {
    ...
    var res = await fetch(TTS_URL, {
      ...
      body: JSON.stringify({
        text: text,
        voiceId: voiceSess.voiceId,
        seed: voiceSess.seed,
        previousRequestIds: voiceSess.previousRequestIds
      })
    });
    ...
    try {
      var rid = res.headers.get("x-request-id") || res.headers.get("request-id");
      if (rid) pushVoiceRequestId(rid);
    } catch (e) {}

    /* Streaming path: hand back a MediaSource URL immediately and pump bytes
       into the SourceBuffer as they arrive, so audio starts on the first
       chunk instead of after the full download. */
    if (MSE_SUPPORTED && res.body && typeof res.body.getReader === "function") {
      ...
      /* Registered out here rather than inside sourceopen: if this chunk is
         cancelled before it ever plays -- which happens on every barge-in --
         sourceopen never fires, so a handler installed in there would never
      ...
      ms.addEventListener("sourceopen", function () {
        ...
        var queue = [];
        /* streamDone = the network reader finished. It must NOT gate pump(),
           or chunks still queued at that moment are dropped and endOfStream()
           never fires -- which stalls the audio element's "ended" event and
           deadlocks the speech queue after the first sentence. Only a hard
           append failure stops us feeding the buffer. */
        var streamDone = false;
        var failed = false;

        function pump() {
          if (failed || sb.updating || queue.length === 0) return;
          try {
          ...
          reader
            ...
            .then(function (r) {
              ...
              if (!firstBytesLogged) {
                ...
                if (isFirst) {
                  console.log(
                    "[VoiceLatency] 1st TTS bytes streamed in " +
    ...
    /* Fallback: fully buffered blob URL (no audio MSE on this browser). */
  ...
  function createSpeechQueue(audio, signal, onChunkStart) {
    ...
    function enqueue(chunk) {
      ...
      var urlPromise = synthesize(spoken || text, signal).catch(function (err) {
    ...
    function drain() {
      if (!firstFlushed) {
        ...
        if (early && early[1].length >= MIN_CHUNK) {
          ...
          cut = space > MIN_CHUNK ? space : FIRST_SOFT_CAP;
        } else if (pending.length >= FIRST_HARD_CAP) {
          cut = FIRST_HARD_CAP;
        }

        if (cut > 0) cut = snapCutOutOfUrl(pending, cut);
        if (cut > 0) {
      ...
      while ((m = re.exec(pending)) !== null) {
        var boundary = re.lastIndex;
        var safe = snapCutOutOfUrl(pending, boundary);
        if (safe < boundary) {
          /* Boundary sits inside a URL that is still streaming in — stop and
             buffer rather than emitting half an address. */
          break;
        }
        if (safe > boundary) {
          /* Boundary sat inside a completed URL — swallow it whole. */
          acc += pending.slice(consumed, safe);
          consumed = safe;
          re.lastIndex = safe;
        } else {
          acc += m[0];
  ...
  /* How long the mic stream is kept warm after the last user releases it.
     Re-opening a mic on a phone costs 300-1500ms, so holding it briefly makes
     the hand-off from "speech detected" to "recording" effectively free. */
  var MIC_IDLE_MS = 20000;

  /**
   * Picks a container/codec this browser can actually record.
   * Safari (all iOS browsers) cannot record audio/webm — passing it to the
   * MediaRecorder constructor throws NotSupportedError, which previously left
   * the mic permanently dead on iPhone and iPad. Empty string means "let the
   * browser choose", which is always safe.
   */
  var RECORDER_MIME = (function () {
  ...
  /** Filename extension matching a recorded blob, so STT sees the right type. */
  function extensionForMime(mime) {
    var m = String(mime || "").toLowerCase();
    if (m.indexOf("mp4") >= 0 || m.indexOf("aac") >= 0) return "mp4";
  ...
  /**
   * One microphone stream and one AudioContext, shared by the recorder, the
   * barge-in listener and the auto-listen watcher.
   *
   * Previously each of those opened its own getUserMedia stream and its own
   * AudioContext. On desktop that is merely wasteful; on mobile it was the
   * cause of both reported symptoms:
   *   - latency: auto-listen detected speech, tore its stream down, then the
   *     recorder opened a *new* one. That re-acquisition costs several hundred
   *     milliseconds on a phone, and the user's first word landed inside it.
   *   - unresponsiveness: the recorder's AudioContext was never closed, and
   *     mobile Safari caps how many can exist (~4). After a few turns every
   *     new AudioContext threw and the mic stopped working entirely.
   *
   * Reference counted, with a short warm window so rapid release/acquire
   * cycles (which is exactly the detect -> record hand-off) never re-prompt or
   * re-open the hardware.
   */
  var SharedMic = (function () {
    var stream = null;
    var pending = null;
    var ctx = null;
    var refs = 0;
    ...
    /** Shared AudioContext, resumed if the browser started it suspended. */
    function getContext() {
      var AC = window.AudioContext || window.webkitAudioContext;
      if (!AC) return null;
      if (!ctx || ctx.state === "closed") ctx = new AC();
      /* Mobile browsers create an AudioContext in "suspended" state until a
         user gesture resumes it. While suspended the analyser only ever
         reports silence, so voice detection never triggers and the mic looks
         dead. Resuming is idempotent and safe to call repeatedly. */
    ...
    function acquire() {
      ...
      refs++;
      if (stream && tracksLive(stream)) return Promise.resolve(stream);
      if (!pending) {
        pending = navigator.mediaDevices.getUserMedia(MIC_CONSTRAINTS).then(
          function (s) {
            stream = s;
            pending = null;
            return s;
    ...
    function release() {
      ...
      idleTimer = setTimeout(hardRelease, MIC_IDLE_MS);
    }

    /** Analyser node fed by the shared stream. Caller disconnects on cleanup. */
    function createAnalyser(fftSize) {
      ...
      return { analyser: analyser, source: source, context: context };
    }

    /** Full teardown — used when the widget closes. */
    function shutdown() {
      refs = 0;
      if (idleTimer) {
        clearTimeout(idleTimer);
  ...
  function createRecorder(onCapture, onReject, onStateChange) {
    ...
    function stop() {
      ...
      /* Disconnect our analyser but leave the stream itself running: the
         shared mic stays warm so the next turn starts instantly instead of
         paying for another getUserMedia. */
      if (analyserSource) {
        try {
    ...
    async function start() {
      try {
        var media = await SharedMic.acquire();
        micHeld = true;
        stream = media;

        /* Codec must be one this browser can record; forcing audio/webm threw
           on iOS and killed the mic. Fall back to the browser default. */
        var rec;
        try {
          rec = RECORDER_MIME
            ? new MediaRecorder(media, { mimeType: RECORDER_MIME })
            : new MediaRecorder(media);
        } catch (e) {
          rec = new MediaRecorder(media);
        }
        var blobMime = rec.mimeType || RECORDER_MIME || "audio/webm";
        recorder = rec;
        chunks = [];
        ...
        rec.onstop = function () {
          var blob = new Blob(chunks, { type: blobMime });
        ...
        var rig = SharedMic.createAnalyser(512);
        if (!rig) {
          console.warn("[mic] no AudioContext available");
          stop();
          return;
        }
        analyserSource = rig.source;
        var node = rig.analyser;
        analyser = node;
  ...
  function createWidget(root) {
    ...
    /** Finds a smooth-scroll controller if the host page exposes one.
     *  Lenis is declared as `const lenis = new Lenis()` in the common snippet,
     *  which creates a lexical binding rather than a window property, so it is
     *  unreachable. Assigning `window.lenis = new Lenis()` instead lets the
     *  widget suspend it cleanly; without that we fall back to pinning body. */
    function getSmoothScroller() {
      var candidates = [window.lenis, window.__lenis, window.smoothScroll];
      for (var i = 0; i < candidates.length; i++) {
        var c = candidates[i];
        if (c && typeof c.stop === "function" && typeof c.start === "function") return c;
      }
      return null;
    }

    var suspendedScroller = null;
    ...
    function setBodyScrollLock(shouldLock) {
      ...
      if (shouldLock) {
        /* Preferred path: ask the smooth-scroll library to pause. Pinning
           <body> underneath Lenis makes it lose its position and snap on
           release, so we only do that when no scroller is reachable. */
        var scroller = getSmoothScroller();
        if (scroller) {
          try {
            scroller.stop();
            suspendedScroller = scroller;
            toggleClass(html, "agx-scroll-lock", true);
            return;
          } catch (e) {
            suspendedScroller = null;
          }
        }
        ...
        if (suspendedScroller) {
          try {
            suspendedScroller.start();
          } catch (e) {}
          suspendedScroller = null;
          return; /* body was never pinned, nothing to restore */
        }
    ...
    /* Cheap signature so we only write when the transcript actually changed,
       rather than on every render during token streaming. */
    var lastPersistSig = "";
    function persistSession() {
      var n = state.messages.length;
      var tail = n ? String(state.messages[n - 1].content || "").length : 0;
      var sig = n + ":" + tail;
    ...
    function buildDom() {
      ...
      /* Lenis (and most smooth-scroll libraries) attach a global wheel/touch
         listener and preventDefault() it, so a nested scroll container never
         sees the gesture and only its scrollbar works. These attributes are
         Lenis's documented opt-out; the variants cover older builds that only
         recognise the split wheel/touch forms. Harmless without Lenis. */
      messages.setAttribute("data-lenis-prevent", "");
      messages.setAttribute("data-lenis-prevent-wheel", "");
      messages.setAttribute("data-lenis-prevent-touch", "");

      /* Belt and braces: Lenis listens on window in the bubble phase, so
         stopping propagation here means it never runs its preventDefault and
         the browser's own scrolling proceeds untouched. Passive listeners --
         we never cancel the event, we only keep it from travelling further. */
      var keepScrollLocal = function (ev) {
        ev.stopPropagation();
      };
      messages.addEventListener("wheel", keepScrollLocal, { passive: true });
      messages.addEventListener("touchmove", keepScrollLocal, { passive: true });
      messages.addEventListener("scroll", function () {
        if (programmaticScroll) return;
        stickToBottom = isNearBottom();
      });

      /* Re-pin when the container itself changes size: dragging a resize
         handle, switching expanded <-> compact, or the on-screen keyboard
         opening on mobile. */
    ...
    var welcomeEffect = createEffect(function () {
      /* Greeting rules:
           - close and reopen the widget      -> no replay (`greeted` in memory)
           - navigate to another page, same tab -> no replay (session flag)
           - refresh                          -> replays (flag cleared on reload)
           - new tab / tab reopened           -> replays (sessionStorage empty)
         The transcript itself is restored on navigation, so the conversation
      ...
      /* Already greeted earlier in this tab, and this load was a navigation
         rather than a refresh -- so carry on silently. */
      if (hasGreetedThisSession()) return null;
    ...
    var bargeInEffect = createEffect(function () {
      ...
      (async function () {
        try {
          stream = await SharedMic.acquire();
          micHeld = true;
          if (stopped) return;
      ...
      return function () {
        ...
        /* Release the shared mic rather than stopping its tracks: another
           watcher (or the recorder we are handing off to) may still need it,
           and keeping it warm is what removes the mobile re-acquire delay. */
        if (micHeld) {
          micHeld = false;
          SharedMic.release();
        }
    ...
    var autoListenEffect = createEffect(function () {
      ...
      (async function () {
        try {
          ...
          var rig = SharedMic.createAnalyser(1024);
          if (!rig) return;
          source = rig.source;
          var analyser = rig.analyser;
          var data = new Uint8Array(analyser.frequencyBinCount);
          ...
          var tick = function () {
            ...
            if (rms >= threshold) {
              ...
              if (now - loudSince >= AUTO_MIN_DURATION_MS) {
                stopped = true;
                /* Hand straight over to the recorder on the live stream.
                   Stopping the tracks here forced a fresh getUserMedia inside
                   recorder.start() — several hundred ms on mobile, during
                   which the user's first word was simply not being recorded.
                   The effect cleanup still runs and releases our reference. */
                if (source) {
                  try {
                    source.disconnect();
                  } catch (e) {}
    ...
    function renderMessages() {
      ...
      for (var i = 0; i < msgs.length; i++) {
        ...
        if (rec.content !== m.content) {
          ...
          if (m.role === "assistant") {
            rec.prose.innerHTML = Markdown.render(linkifyForDisplay(m.content || "\u2026"));
          } else {
    ...
    function render() {
      ...
      /* overlay + backdrop — blur/lock the whole page any time the widget
         is open at all, including the minimized (docked) window, so
         nothing behind it is reachable. */
      toggleClass(dom.overlay, "agx-open", open);
      toggleClass(dom.backdrop, "agx-visible", open);
      setBodyScrollLock(open);
      try { document.body.classList[open ? "add" : "remove"]("agx-open-lock"); } catch (e) {}
      ...
      /* orb */
      var stacked = expanded && state.isMobile;
      var scale = expanded ? 1 : Math.min(1, Math.max(0.5, state.size.height / 520));
      var glowSize, coreSize, iconSize;

      if (stacked) {
        /* On a phone the stage is capped at 38% of the panel, but the orb was
           still drawn at full desktop size (200px core / 320px glow). It
           overflowed across the status text and the button row, which is what
           made the controls unreachable. Size it from the actual viewport. */
        var budget = Math.min(window.innerWidth, window.innerHeight * 0.32);
        coreSize = Math.max(72, Math.min(132, Math.round(budget * 0.36)));
        glowSize = Math.round(coreSize * 1.5);
        iconSize = Math.round(coreSize * 0.32);
      } else if (expanded) {
        glowSize = 320;
        coreSize = 200;
        iconSize = 64;
      } else {
        glowSize = Math.round(160 * scale);
        coreSize = Math.round(100 * scale);
      ...
      dom.orbRings.forEach(function (ring, idx) {
        ...
        if (showRings) {
          /* Derived from the orb rather than hardcoded: the old 240/300/360px
             rings were wider than a phone screen. */
          var base = stacked ? coreSize * 1.2 : 240;
          var step = stacked ? coreSize * 0.3 : 60;
          var size = Math.round(base + idx * step + level * (stacked ? 40 : 80));
    ...
    /* -- auto-scroll on new content --------------------------------------
       Sticks to the bottom as content streams in (message count AND the
       growing text of the in-progress reply). Stops auto-following the
       moment the user scrolls up to read earlier messages, and resumes
       once they're back near the bottom or a new message is sent. */
    var stickToBottom = true;

    /* Set while we move the scrollbar ourselves, so our own scrolling isn't
    ...
    /* Pins the history to the newest message. Runs a second pass on the next
       frame because a single assignment lands short whenever the content is
       still growing — streaming text, a just-swapped mode, or a resize all
       change scrollHeight after this tick, which is what previously left the
       docked window stuck mid-history. */
    function autoScroll() {
      if (!dom.messages || !stickToBottom) return;
      var m = dom.messages;
      programmaticScroll = true;
      m.scrollTop = m.scrollHeight;

      requestAnimationFrame(function () {
        if (stickToBottom && m.scrollHeight - m.scrollTop - m.clientHeight > 1) {
          m.scrollTop = m.scrollHeight;
        }
    ...
    function syncTopOffset() {
      ...
      if (headerBottom > 0) {
        ...
        /* Keep the dim/blur below the navbar so it stays sharp and clickable.
           Doing it by geometry rather than z-index means it behaves the same
           on every page, regardless of how the host stacks its header. */
        root.style.setProperty(
          "--agx-backdrop-top",
          KEEP_NAVBAR_CLEAR ? Math.round(headerBottom) + "px" : "0px"
        );
      } else {
        root.style.removeProperty("--agx-top-offset"); // fall back to the CSS default
        root.style.setProperty("--agx-backdrop-top", "0px");
      }
    }
  ...
  /* ----------------------------------------------------------------------
     Build marker. Deploys in this project have repeatedly been masked by CDN
     caching, so expose which build is actually running. In the browser
     console (or Safari/Chrome remote inspector on a phone) run:

         __AGX_BUILD          -> version + chosen audio codec
         __AGX_MIC()          -> live mic / AudioContext state

     If __AGX_BUILD is undefined, the page is running an OLD cached file and
     no amount of further code changes will show up until it is purged.
     ---------------------------------------------------------------------- */
  window.__AGX_BUILD = {
    version: "mobile-mic-1",
    recorderMime: RECORDER_MIME || "(browser default)",
    sharedMic: true,
    notes: "shared mic stream + shared AudioContext, iOS-safe codec"
  };

  window.__AGX_MIC = function () {
    var ctx = null;
    try {
      ctx = SharedMic.getContext();
    } catch (e) {}
    return {
      micOpen: SharedMic.isOpen(),
  ...
  function mount() {
    /* Must run before createWidget reads the stored transcript. */
