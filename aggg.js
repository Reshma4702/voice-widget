...
(function () {
  ...
  /* Webflow can load project-level and page-level footer code together.
     Keep a second include from mounting a duplicate widget on top of the first. */
  if (window.__agilisiumVoiceWidgetLoaded) return;
  window.__agilisiumVoiceWidgetLoaded = true;
  ...
  var VT = {
    ...
    mark: function (label) {
      ...
      console.log(
        "%c[VoiceLatency] " + label.padEnd(32) + " +" + String(delta).padStart(5) + "ms   (total " + total + "ms)",
        "color:#0ea5e9",
      );
    },
    summary: function () {
  ...
  var _TTS_TLD = "com|io|ai|org|net|co|dev|health|life|edu|gov|app|tech|uk|us|in|eu";
  var _TTS_DOMAIN_RE = new RegExp(
    "(?:^|[^@\\w/.])\\b[a-z0-9](?:[a-z0-9-]*[a-z0-9])?(?:\\.[a-z0-9-]+)*\\.(?:" + _TTS_TLD + ")\\b(?:\\/[^\\s)]*)?",
    "gi",
  );
  var LINK_SPEECH_PHRASE = "You can check it here";
  var _TTS_URLISH_LABEL_RE = new RegExp(
    "^(?:(?:https?:\\/\\/|www\\.)\\S+|[a-z0-9](?:[a-z0-9-]*[a-z0-9])?(?:\\.[a-z0-9-]+)*\\.(?:" + _TTS_TLD + ")(?:\\/\\S*)?)$",
    "i",
  );
  function linkSpeechReplacement(value) {
    var trailingMatch = String(value).match(/[.,!?;:]+$/);
    return " " + LINK_SPEECH_PHRASE + (trailingMatch ? trailingMatch[0] : "") + " ";
  }
  function stripForSpeech(text) {
    if (!text) return "";
    var out = String(text);
    out = out.replace(/\[([^\]]+)\]\((?:https?:\/\/|www\.|mailto:)?[^)]+\)/gi, function (_m, label) {
      var cleanLabel = String(label).trim();
      return _TTS_URLISH_LABEL_RE.test(cleanLabel)
        ? " " + LINK_SPEECH_PHRASE + " "
        : " " + cleanLabel + "; " + LINK_SPEECH_PHRASE + " ";
    });
    out = out.replace(/\bhttps?:\/\/[^\s<>'"\])}]+[.,!?;:]?/gi, linkSpeechReplacement);
    out = out.replace(/\bwww\.[^\s<>'"\])}]+[.,!?;:]?/gi, linkSpeechReplacement);
    out = out.replace(_TTS_DOMAIN_RE, function (m) {
      // preserve leading boundary char captured by the non-lookbehind pattern
      var lead = m.charAt(0);
      var value = /[a-z0-9]/i.test(lead) ? m : m.slice(1);
      return (/[a-z0-9]/i.test(lead) ? "" : lead) + linkSpeechReplacement(value);
    });
    out = out.replace(/\bmailto:/gi, "");
    out = out.replace(/[*_`]+/g, "");
    out = out.replace(/(?:\s*You can check it here[.,;:!?]*\s*){2,}/gi, " " + LINK_SPEECH_PHRASE + ". ");
  ...
  var DISPLAY_LINK_LABEL = "check here";
  ...
  function linkifyForDisplay(text) {
    ...
    out = out.replace(/\[([^\]]+)\]\(([^)\s]+)\)/g, function (_m, label, url) {
      var cleanLabel = String(label).trim();
      var href = /^(?:https?:\/\/|mailto:|www\.)/i.test(url) ? normalizeHref(url) : url;
      return _TTS_URLISH_LABEL_RE.test(cleanLabel)
        ? "[" + DISPLAY_LINK_LABEL + "](" + href + ")"
        : "[" + cleanLabel + "](" + href + ")";
    });
    var placeholders = [];
    ...
    function bareReplace(match) {
      var trailingMatch = match.match(/[.,!?;:)]+$/);
      var trailing = trailingMatch ? trailingMatch[0] : "";
      var core = trailing ? match.slice(0, -trailing.length) : match;
      var md = "[" + DISPLAY_LINK_LABEL + "](" + normalizeHref(core) + ")";
      placeholders.push(md);
      return "\u0000LINK" + (placeholders.length - 1) + "\u0000" + trailing;
    }
    out = out.replace(/\bhttps?:\/\/[^\s<>'"\])}]+/gi, bareReplace);
    out = out.replace(/\bwww\.[^\s<>'"\])}]+/gi, bareReplace);
    out = out.replace(_TTS_DOMAIN_RE, function (m) {
      var lead = m.charAt(0);
      var value = /[a-z0-9]/i.test(lead) ? m : m.slice(1);
      var leadOut = /[a-z0-9]/i.test(lead) ? "" : lead;
      return leadOut + bareReplace(value);
    });
    out = out.replace(/\u0000LINK(\d+)\u0000/g, function (_m, i) {
      return placeholders[Number(i)] || "";
    });
    return out;
  }

  function speechProtectedRanges(text) {
    var ranges = [];
    var patterns = [
      /\[[^\]]*\]\((?:https?:\/\/|www\.|mailto:)?[^)]+\)/gi,
      /\bhttps?:\/\/\S+/gi,
      /\bwww\.\S+/gi,
      new RegExp(_TTS_DOMAIN_RE.source, "gi"),
    ];
    patterns.forEach(function (pattern) {
      var match;
      while ((match = pattern.exec(text)) !== null) {
        var end = match.index + match[0].length;
        while (end > match.index && /[.,!?;:]/.test(text.charAt(end - 1))) end--;
  ...
  function openLinkTailStart(text) {
    var markdownStart = text.lastIndexOf("](");
    if (markdownStart >= 0 && text.indexOf(")", markdownStart + 2) < 0) return markdownStart;
    var re = new RegExp(
      "(?:https?|https?:|https?:\\/|https?:\\/\\/|www\\.)[^\\s]*$|\\b[a-z0-9](?:[a-z0-9-]*[a-z0-9])?(?:\\.[a-z0-9-]+)*\\.(?:" + _TTS_TLD + ")(?:\\/[^\\s]*)?$",
      "i",
    );
  ...
  function safeSpeechBoundary(text, minLength, firstChunk) {
    var ranges = speechProtectedRanges(text);
    var openStart = openLinkTailStart(text);
    var limit = openStart >= 0 ? openStart : text.length;
    for (var i = 0; i < limit; i++) {
      var boundary = firstChunk ? /[.!?\n,;:—-]/.test(text.charAt(i)) : /[.!?\n]/.test(text.charAt(i));
      if (!boundary || i + 1 < minLength) continue;
      var protectedAt = ranges.some(function (range) { return i >= range.start && i < range.end; });
      if (!protectedAt) return i + 1;
    }
    return -1;
  }

  function safeSpeechCap(text, cap, minLength) {
    var openStart = openLinkTailStart(text);
    var limit = Math.min(cap, openStart >= 0 ? openStart : text.length);
    if (limit < minLength) return -1;
    var ranges = speechProtectedRanges(text);
    for (var i = limit; i >= minLength; i--) {
      if (!/\s/.test(text.charAt(i - 1))) continue;
      var inside = ranges.some(function (range) { return i > range.start && i < range.end; });
      if (!inside) return i;
    }
    return openStart < 0 && text.length >= cap ? limit : -1;
  }
  ...
  var Markdown = (function () {
    ...
    function inline(src) {
      ...
      /* gfm autolinks */
      text = text.replace(/(^|[\s(])((?:https?:\/\/|www\.)[^\s<]+[^\s<.,:;"')\]])/g, function (_, pre, url) {
        var href = /^www\./i.test(url) ? "http://" + url : url;
        return pre + '<a href="' + safeUrl(href) + '" target="_blank" rel="noopener noreferrer">' + url + "</a>";
      });
        ...
        if (listMatch) {
          ...
          while (i < lines.length) {
            ...
            if (m2 && m2[1].length <= baseIndent + 1) {
              ...
              while (i < lines.length) {
                if (!lines[i].trim()) {
                  if (i + 1 < lines.length && /^\s+\S/.test(lines[i + 1]) && lines[i + 1].search(/\S/) > baseIndent) {
                    nested.push("");
                    i++;
                    continue;
                  }
  ...
  async function synthesize(text, signal) {
    ...
    var speakable = stripForSpeech(text);
    if (!speakable) return "";
    ...
    var res = await fetch(TTS_URL, {
      ...
      signal: signal,
      headers: authHeaders({ "Content-Type": "application/json" }),
      body: JSON.stringify({ text: speakable, voice_id: TTS_VOICE_ID }),
    });
    if (!res.ok) throw new Error("tts " + res.status);
    var effectiveVoice = res.headers.get("x-tts-voice-id");
    if (effectiveVoice && effectiveVoice !== TTS_VOICE_ID) {
      throw new Error("TTS provider returned an unexpected voice");
    }
  ...
  /** Cheap reachability probe against the STT endpoint. Uses HEAD + a 3s timeout
   *  so a slow/CORS-noisy OPTIONS preflight can't produce false "unavailable". */
  async function pingVoiceService(signal) {
    var timer = null;
    var ac = new AbortController();
    var onAbort = function () { try { ac.abort(); } catch (e) {} };
    if (signal) signal.addEventListener("abort", onAbort);
    timer = window.setTimeout(function () { try { ac.abort(); } catch (e) {} }, 3000);
    try {
      var res = await fetch(STT_URL, { method: "HEAD", mode: "cors", signal: ac.signal, headers: authHeaders() });
      return res.status < 500;
    } catch (e) {
      // Treat network / CORS errors as "unknown" (return null) so the caller can
      // avoid flapping the pill on a single blip.
      return null;
    } finally {
  ...
  function createSpeechQueue(audio, signal, onChunkStart) {
    ...
    var records = []; // { text, revealed }

    function reveal(rec) {
      if (!rec || rec.revealed) return;
      rec.revealed = true;
      try { if (onChunkStart) onChunkStart(rec.text); } catch (e) {}
    }

    signal.addEventListener("abort", function () {
      cancelled = true;
      try { audio.pause(); } catch (e) {}
      // Reveal any pending un-shown text so the transcript stays complete.
    ...
    function enqueue(chunk) {
      ...
      var rec = { text: chunk, revealed: false };
      records.push(rec);
      ...
      chain = chain.then(async function () {
        if (cancelled) { reveal(rec); return; }
        var url = await urlPromise;
        if (cancelled) { reveal(rec); return; }
        if (!url) {
          // TTS failed for this chunk — reveal text so the bubble isn't lost,
          // but preserve voice-first ordering (this runs inside the chain, so
          // prior audio has already played).
          reveal(rec);
          return;
        }
        ...
        await new Promise(function (resolve) {
          ...
          function finish() {
            ...
            // Safety net: if `playing` never fired (very short blob), reveal.
            reveal(rec);
    ...
    function drain() {
      if (!firstFlushed) {
        var firstBoundary = safeSpeechBoundary(pending, MIN_CHUNK, true);
        if (firstBoundary > 0) {
          var head = pending.slice(0, firstBoundary);
          pending = pending.slice(head.length);
          enqueue(head);
          firstFlushed = true;
        } else if (pending.length >= FIRST_SOFT_CAP) {
          var cut = safeSpeechCap(pending, FIRST_SOFT_CAP, MIN_CHUNK);
          if (cut > 0) {
          ...
          var hardCut = safeSpeechCap(pending, FIRST_HARD_CAP, MIN_CHUNK);
          if (hardCut > 0) {
            var hard = pending.slice(0, hardCut);
            pending = pending.slice(hardCut);
            enqueue(hard);
            firstFlushed = true;
          }
        }
      ...
      var boundary = safeSpeechBoundary(pending, MIN_SENTENCE, false);
      while (boundary > 0) {
        var sentence = pending.slice(0, boundary);
        pending = pending.slice(boundary);
        enqueue(sentence);
        boundary = safeSpeechBoundary(pending, MIN_SENTENCE, false);
      }
    }
    ...
    return {
      ...
      flush: async function () {
        ...
        await chain;
        // Force-reveal any still-unrevealed tail (covers short final blobs).
        records.forEach(reveal);
      },
      cancel: function () {
        cancelled = true;
  ...
  function createConnectionMonitor(onChange) {
    ...
    async function doCheck() {
      ...
      // Don't flash "checking" once we're already connected — reduces flapping.
      if (status !== "connected") set("checking");
      ...
      // ok === false (definite failure) or null (network/CORS unknown)
      consecutiveFail += 1;
      // Only surface a visible failure state after two consecutive failures,
      // so one transient blip doesn't repaint the pill.
      if (consecutiveFail >= 2) {
        set(ok === false ? "unavailable" : status === "connected" ? "connected" : "checking");
      }
      return false;
    }

    // Coalesce overlapping check() calls + 5s cooldown once we have a result.
    function check() {
      if (checkInFlight) return checkInFlight;
      if (lastCheck && Date.now() - lastCheck < 5000 && status === "connected") {
        return Promise.resolve(true);
      }
      checkInFlight = doCheck().finally(function () { checkInFlight = null; });
      return checkInFlight;
    }

    function scheduleReconnect() {
      clearRetry();
      // Only show "reconnecting" pill after we've actually failed twice.
      if (consecutiveFail >= 2) set("reconnecting");
      var next = Math.min(attempt + 1, 99);
      var delay = Math.min(30000, 1000 * Math.pow(2, Math.min(attempt, 5)));
      set(undefined, next);
      retryTimer = window.setTimeout(async function () {
        retryTimer = null;
        var ok = await check();
        if (!ok && active) scheduleReconnect();
      }, delay);
    }
  ...
  function createWidget(root) {
    ...
    function setPageLocked(locked) {
      if (locked && !pageLock) {
        pageLock = { x: window.scrollX || 0, y: window.scrollY || 0 };
        document.documentElement.classList.add("agx-page-locked");
        document.body.classList.add("agx-page-locked");
      } else if (!locked && pageLock) {
        var previous = pageLock;
        pageLock = null;
        document.documentElement.classList.remove("agx-page-locked");
        document.body.classList.remove("agx-page-locked");
    ...
    function buildDom() {
      ...
      panel.addEventListener("pointerdown", function (ev) {
        ...
        if (ev.target.closest("button, a, input, textarea, [data-no-drag], [data-resize-handle]")) return;
        beginDrag(ev);
      });
      ...
      brand.innerHTML = iconMarkup("sparkles", "agx-brand-icon", 1.5) + '<span class="agx-brand-text"></span>';
      ...
      orbCore.innerHTML = '<div class="agx-orb-sheen"></div>' + iconMarkup("sparkles", "agx-orb-icon", 1.5);
    ...
    /* Tap handler that works reliably on iOS Safari — pointerup preempts the
     * synthetic click, avoids the 300ms tap delay, and can't be swallowed by
     * ancestor pointerdown/drag handlers on the shell. */
    function bindTap(node, handler) {
      ...
      // Fallback for browsers that don't fire pointerup (or when it's suppressed).
    ...
    function unlockAudioOnGesture() {
      ...
      try {
        ensureAudio();
        // A muted play() during a user gesture unlocks HTMLAudio on iOS.
        var prev = audio.muted;
        audio.muted = true;
        var p = audio.play();
        if (p && p.then) p.then(function () {
          try { audio.pause(); audio.muted = prev; } catch (e) {}
        }).catch(function () { try { audio.muted = prev; } catch (e) {} });
      } catch (e) {}
    }
    ...
    async function sendMessage(rawText, opts) {
      ...
      try {
        ...
        var queue =
          voice && !state.voiceMuted && audio ? createSpeechQueue(audio, speechController.signal, onChunkSpoken) : null;
        ...
        var outgoing = wasInterrupted
          ? history.concat([{ role: "assistant", content: INTERRUPTION_PROMPT, ts: Date.now() }], [userMsg])
    ...
    var welcomeEffect = createEffect(function () {
      ...
      (async function () {
        try {
          ...
          console.log("[welcome] static welcome played (t=" + Math.round(performance.now() - t0) + "ms)");
    ...
    var bargeInEffect = createEffect(function () {
      if (!state.open || (!state.speaking && !state.thinking) || recorder.isListening() || state.micMuted) return null;
    ...
    function runEffects() {
      ...
      bargeInEffect([state.open, state.speaking, state.thinking, recorder.isListening(), state.micMuted].join("|"));
      autoListenEffect([state.open, state.speaking, state.thinking, recorder.isListening(), state.micMuted].join("|"));
    }
    ...
    function renderConnection() {
      ...
      var variants = {
        ...
        reconnecting: {
          label: "Reconnecting" + (attempt > 1 ? " (#" + Math.min(attempt, 3) + (attempt > 3 ? "+" : "") + ")" : "\u2026"),
          icon: "loader-circle",
          spin: true,
          pulse: true,
        },
    ...
    function render() {
      ...
      dom.orbCore.style.boxShadow = "0 0 " + (40 + level * 120) + "px rgba(20,184,166," + (0.4 + level * 0.5) + ")";
      ...
      setText(
        dom.statusTitle,
        state.speaking ? "Speaking\u2026" : listening ? "Listening\u2026" : state.thinking ? "Thinking\u2026" : "Ready",
      );
      setText(dom.statusSub, listening ? "I'll respond when you pause" : state.micMuted ? "Tap the mic to begin" : "");

      /* mic + voice buttons */
      setSmallIcon(dom.micBtn, state.micMuted ? "mic-off" : "mic");
      dom.micBtn.setAttribute("aria-label", state.micMuted ? "Unmute microphone" : "Mute microphone");
  ...
  function mount() {
    ...
    /* Replace any stale instance left by an older duplicate Webflow embed. */
