...
(function () {
  ...
  /* Greeting is spoken once per browser tab. sessionStorage (not localStorage)
     so it clears when the tab closes, and survives navigation between pages --
     which is the case that mattered here: without it the welcome replayed on
     every Webflow page load as the visitor browsed the site. */
  ...
  function createWidget(root) {
    ...
    var welcomeEffect = createEffect(function () {
      /* Note: `greeted` is NOT reset on close any more, so shutting the widget
         and reopening it within the same page does not replay the greeting. */
      if (!state.open) return null;
      if (greeted) return null;

      greeted = true;

      /* The transcript is always seeded, so a reopened or freshly loaded panel
         is never blank -- only the spoken audio is rationed to once per tab. */
    ...
    function measureHostHeader() {
      ...
      try {
        ...
        for (var i = 0; i < candidates.length; i++) {
          ...
          /* An explicit marker wins outright: if the site says this is the
             navbar, believe it and skip every heuristic below. */
          if (node.hasAttribute("data-agx-navbar")) {
            if (rect.bottom > bottom) bottom = rect.bottom;
            continue;
          }

          /* "absolute" matters as much as fixed/sticky: a transparent nav laid
             over a hero (common on a homepage, while inner pages use a solid
             fixed bar) is usually absolutely positioned. Excluding it made the
             widget treat the two page types differently. An absolute nav does
             scroll away, so it only counts while still on screen. */
          var pos = cs.position;
          if (pos !== "fixed" && pos !== "sticky" && pos !== "absolute") continue;
          if (pos === "absolute" && rect.bottom <= 0) continue;    // scrolled out of view
